...
// ============================================================
//  NUT-KAIZEN — LOGROS (shonen pintado)
...
export default function Logros({ onBack }: { onBack?: () => void }) {
  ...
  const logros: Logro[] = [
    { id: "primer", icon: Dumbbell, titulo: "PRIMER PASO", desc: "Completa tu primer entrenamiento", valor: prog.entrenos, meta: 1, tone: "var(--amber)" },
    { id: "constante", icon: Flame, titulo: "GUERRERO CONSTANTE", desc: "Completa 50 entrenamientos", valor: prog.entrenos, meta: 50, tone: "var(--red)" },
    { id: "bestia", icon: Trophy, titulo: "MODO BESTIA", desc: "Completa 100 entrenamientos", valor: prog.entrenos, meta: 100, tone: "var(--gold)" },
    { id: "nivel", icon: Zap, titulo: "ASCENSIÓN", desc: "Alcanza el nivel 25", valor: prog.nivel, meta: 25, tone: "#9b6bd2" },
    { id: "nutricion", icon: Salad, titulo: "NUTRICIÓN EN ORDEN", desc: "Registra 10 comidas", valor: comidas.length, meta: 10, tone: "var(--teal)" },
          ...
          {logros.map((l) => {
            ...
            return (
              <div key={l.id} className={`medalla ${logrado ? "on" : ""}`}>
                <div className="medalla-ic" style={{ borderColor: logrado ? l.tone : "var(--ink)", color: logrado ? l.tone : "#5a4a3a" }}>
...
const CSS = `
  * { box-sizing: border-box; margin: 0; }
  ...
  .medalla { display:flex; align-items:center; gap:13px; background:linear-gradient(160deg,var(--panel2),var(--panel));
    border:2px solid var(--ink); border-radius:8px; padding:13px; opacity:.7; transition:.15s; }
  .medalla.on { opacity:1; background:linear-gradient(160deg,var(--panel2),var(--panel)); box-shadow:3px 3px 0 #00000044; }
  .medalla-ic { width:48px; height:48px; border-radius:8px; border:2px solid; background:var(--bg2);
  ...
  .medalla-bar { height:8px; background:var(--bg1); border:2px solid var(--ink); border-radius:3px; overflow:hidden; margin-top:4px; }
